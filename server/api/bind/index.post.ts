...
const requestSchema = z.object({
  username: z.string().length(9).startsWith("s"), password: z.string(), userId: z.string(),
})
...
export default defineEventHandler(async (event) => {
  ...
  const tsimsRawLoginResult = await $fetch<string>('http://101.230.1.173:6300/php/login.php', {
      ...
      }, body: `username=${result.data.username}&password=${result.data.password}`, onResponse(context) {
  ...
  if (tsimsLoginResult["status"] != "ok") {
    throw createError({
      statusCode: 403, statusMessage: 'Login Credentials Incorrect',
    })
  }
  ...
  const tsimsRawInfoResult = await $fetch<string>('http://101.230.1.173:6300/php/init_info.php', {
    method: 'POST', credentials: 'include', headers: {
      'content-type': 'application/x-www-form-urlencoded', 'Cookie': sessionCookie
    },
  })
  ...
  await useStorage().setItem(continueToken, {
    "userId": result.data.userId, "name": tsimsInfoResult.studentname, "studentId": tsimsInfoResult.studentid
  })
  return {
    "token": continueToken, "name": tsimsInfoResult.studentname, "studentNumber": tsimsInfoResult.studentNo
  }
})
